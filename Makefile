...
ifeq ($(PLAT),Windows)
	./setup.bat $(ARCH) $(DEP_DIR) bld/$(DISC) $(PROJECT_TYPE)
else
	./setup.sh $(ARCH) $(DEP_DIR) bld/$(DISC) $(PROJECT_TYPE)
endif

compile-dependencies:
	(cd dep/glfw/bld/$(DISC)/ && $(MAKE_TOOL))
	(cd dep/portaudio/bld/$(DISC)/ && $(MAKE_TOOL))
	(cd dep/assimp/bld/$(DISC)/ && $(MAKE_TOOL))
	(cd dep/freetype/bld/$(DISC)/ && $(MAKE_TOOL))
